    ...
    onStart: async function ({ api, event }) {
        ...
        try {
            api.getUserInfo(targetID, async (err, obj) => {
                ...
                // আপনার দেওয়া নির্দিষ্ট পাথ ব্যবহার করা হলো
                const fontPath = path.join(process.cwd(), "Func", "hindsiligury", "HindSiliguri-Light.ttf");

                if (fs.existsSync(fontPath)) {
                    registerFont(fontPath, { family: "BanglaFont" });
                    debugInfo += `✅ Font Loaded Successfully!\n`;
                } else {
                    debugInfo += `❌ Font not found at specified path!\n`;
                }
                ...
                ctx.clip();
                try {
                    const avatarBuffer = (await axiosModule.get(avatarURL, { responseType: 'arraybuffer' })).data;
